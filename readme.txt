...
* Bug fix: CSS optimizer gracefully handles invalid selectors such as `,{` in the [Skeptical theme](http://www.woothemes.com/2010/09/skeptical/).
...
* [[via report](http://wordpress.org/support/topic/plugin-speedy-smilies-works-in-graphene-theme-but-not-in-matala)] Bug fix: CSS optimizer gracefully handles zero-length comments  such as those in the [Matala theme](http://wordpress.org/extend/themes/matala).  
...
* [[via report](http://wordpress.org/support/topic/plugin-speedy-smilies-smilies-not-visible)] Bug fix: CSS generated by Speedy Smilies uses the `!important` declaration to prevent conflicts with style rules in your WordPress theme.
