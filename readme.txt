...
* CSS optimizer handles zero-length comments such as those in the Matala theme ([reported here](http://wordpress.org/support/topic/plugin-speedy-smilies-works-in-graphene-theme-but-not-in-matala)).  
...
* Bug fix: CSS generated by Speedy Smilies uses the `!important` declaration to prevent conflicts with style rules in your WordPress theme ([reported here](http://wordpress.org/support/topic/plugin-speedy-smilies-smilies-not-visible)).
